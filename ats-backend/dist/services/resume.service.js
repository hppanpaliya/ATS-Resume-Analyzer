...
class ResumeService {
    ...
    async exportToPDF(resumeId, userId) {
        const resume = await prisma.resume.findFirst({
            where: { id: resumeId, userId, deletedAt: null },
    ...
    async exportToWord(resumeId, userId) {
        try {
            const resume = await prisma.resume.findFirst({
    ...
    async parseResumeWithAI(text, userId) {
    ...
    generateFormattedHTML(structuredResume, template = null) {
